...
window.addEventListener("load", () => {
  ...
  if (!staffCardsContainer.innerHTML) {
    staff.forEach((staffMember) => {
      ...
      card.classList.add("selectstaff");
      card.setAttribute("data-id", staffMember.id);
      ...
      email.textContent = staffMember.email;
    ...
    if (currentStep === 1) {
      ...
      if (selectedDate && selectedTime) {
        ...
        resultContainer.innerHTML = `
          ...
          <p ><strong>Total Price: </strong>  <span class="price">$${totalPrice.toFixed(2)} </span> </p>
        `;
  ...
  const selectedStaffCards = document.querySelectorAll(".selectstaff.selected");
  selectedStaffCards.forEach((staffCard) => {
    staffCard.classList.remove("selected");
  });

  const selectedServiceCards = document.querySelectorAll(".selectservice.selected");
  selectedServiceCards.forEach((serviceCard) => {
